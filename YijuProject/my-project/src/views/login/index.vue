  ...
  <div class="login-wrap">
    ...
    <div class="form-wrap">
      ...
      <el-form
        ...
        :model="form"
        :rules="rules">
        <el-form-item prop="acnumber">
          <el-input v-model="form.acnumber" placeholder="请输入您的账号"></el-input>
        </el-form-item>
        <el-form-item prop="code" style="margin-bottom:10px;">
          <el-input type="password" v-model="form.code" placeholder="请输入您的密码" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        code: [{ required: true, message: "请输入您的密码", trigger: "blur" }]
      },
  ...
  mounted() {
    // 注册页返回登陆页的时候，自动填充注册的用户。否则没有返回参数的话，就为空
    this.form.acnumber = this.$route.query.username || "";
    this.form.code = this.$route.query.password || "";
  },
  methods: {
    handleLogin() {
    ...
    submitLogin() {
      ...
      axios({
        method: "POST",
        url: `http://127.0.0.1:8080/getUserLogInInfo?username=${
          this.form.acnumber
        ...
        .then(res => {
          ...
          if (res.data.uid) {
            ...
            setTimeout(() => { //定时器，延迟1.5s再跳转
              this.$router.push({
                name: "home",
                // 登陆成功自动跳转到首页，携带三个参数
                query: {
                  uid: res.data.uid, //用户id
                  identity: res.data.identity, //用户身份标识
                  username: this.form.acnumber //用户账号
                }
              });
