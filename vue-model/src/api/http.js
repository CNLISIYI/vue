...
instance.interceptors.response.use(function (response) {
  // let _n = router.app._route.name;
  // if (_n == 'home' || _n == 'watchhome' || _n == 'helphome' || _n == 'helpdetail') {
  //   return response.data;
  // } else {
  //   if (response.data.code == 1001) {
  //     cookies.set("authenticate", '');
  //     setTimeout(() => {
  //       router.push({
  //         name: "login",
  //         params: {
  ...
  Message.error(state.Lans.login.wrong);
  console.log('拦截器报错');
...
export default function (method, url, data = null) {
  ...
  if (method == 'post') {
    ...
    //   params: data,
    //   headers: {
    //     'Authenticate': cookies.get('authenticate')
    //   }
    // })
